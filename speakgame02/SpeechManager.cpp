	...
	while (getline(ifs, date))
	{
		...
		while (true)
		{
			...
			if (pos == -1) break;
				...
				string subdate = date.substr(start, pos-start);//末尾置-初始位置
				v.push_back(subdate);
				start = pos + 1;
		}
		this->m_Record.insert(make_pair(index, v));//make_pair与pair不同
		index++;
	}
	ifs.close();
}
...
void SpeechManager::showRecord()
{
	...
	{
		cout << "第" << it->first + 1 << "届:  " << "冠军编号:" << it->second[0] << "姓名:" << it->second[1] << "成绩:" << it->second[2] << " "
			<< "亚军编号:" << it->second[3] << "姓名:" << it->second[4] << "成绩:" << it->second[5] << " "
			<< "季军编号:" << it->second[6] << "姓名:" << it->second[7] << "成绩:" << it->second[8] << endl;
	}
